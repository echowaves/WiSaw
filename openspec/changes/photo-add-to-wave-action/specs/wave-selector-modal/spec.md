...
The system SHALL provide a modal overlay component that displays the user's waves in a searchable scrollable list for selecting a wave to assign a photo to.

#### Scenario: Modal opens with user's waves
- **WHEN** the wave selector modal is opened
- **THEN** the user's waves are fetched via `listWaves` query
...
- **THEN** the wave list is filtered client-side to show only waves whose name contains the search text (case-insensitive)
