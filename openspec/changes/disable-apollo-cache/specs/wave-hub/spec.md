...
The system SHALL re-fetch the waves list and ungrouped photo count from the API every time the Waves screen gains focus, ensuring wave names, photo counts, thumbnails, and the ungrouped badge reflect the latest server state. The refresh SHALL preserve the current sort order. The system SHALL prevent concurrent duplicate fetches using a ref-based loading guard, and SHALL skip the initial debounced-search effect on mount so that only `useFocusEffect` triggers the first load. Thumbnail URLs SHALL be obtained from the `photos` field of the `listWaves` query response, eliminating separate per-wave thumbnail queries. `WaveCard` thumbnail cache keys SHALL be derived from the photo URL (not the array position index) so that thumbnails update immediately when photos are added or removed.
...
- **THEN** `WaveCard` SHALL derive each `CachedImage` `cacheKey` from the photo URL filename, NOT from the array position index
