...
**Reason**: The app only uses foreground location. The global watcher runs while the app is in the foreground, which is sufficient.
**Migration**: No migration needed. Background location was optional and never fully implemented.

### Requirement: Motion Detection Support
**Reason**: Motion sensors are not used by any current feature. The watcher handles location updates.
**Migration**: No migration needed. Can be re-added if a motion-dependent feature is introduced.
