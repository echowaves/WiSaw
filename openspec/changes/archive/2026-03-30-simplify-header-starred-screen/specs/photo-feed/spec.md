...
### Requirement: Segment switching between Global and Starred
**Reason**: The segment control is removed from the header. Starred content is now a separate screen accessible via the drawer. The Global feed no longer has segments.
**Migration**: Users access starred content via the "Starred" drawer menu item instead of the segment toggle in the header.
