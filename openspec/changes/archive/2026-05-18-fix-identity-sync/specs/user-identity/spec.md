...
#### Scenario: UUID atom updated after successful attach
- **WHEN** the user submits the attach form and `registerSecret` mutation succeeds
- **THEN** the system SHALL call `setUuid()` with the new UUID returned from the backend
- **THEN** the system SHALL call `setNickName()` with the new nickname
- **THEN** all subscribers to `identityChangeBus` SHALL re-fetch data using the correct (non-empty) UUID

#### Scenario: Wave grouping recognizes new identity immediately
- **WHEN** the user establishes identity and then triggers auto-group (or navigates to Waves)
- **THEN** the system SHALL fetch ungrouped photos and waves using the new UUID
- **THEN** the ungrouped photos card SHALL display photos belonging to the new identity

#### Scenario: Photo feed recognizes new identity immediately
- **WHEN** the user establishes identity
- **THEN** the `PhotosList` screen SHALL re-fetch the geo feed using the new UUID
- **THEN** the feed SHALL display photos for the new identity (not empty or stale data)

#### Scenario: Friend list recognizes new identity immediately
- **WHEN** the user establishes identity
- **THEN** the `FriendsList` screen SHALL re-fetch friendships using the new UUID
- **THEN** the friend list SHALL display friends for the new identity
