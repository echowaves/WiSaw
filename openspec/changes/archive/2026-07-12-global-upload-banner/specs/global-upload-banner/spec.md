...
The `GlobalUploadBanner` SHALL consume `UploadContext` and `STATE.netAvailable` directly. It SHALL NOT receive upload state as props from a parent screen.
...
- **THEN** it SHALL read `STATE.netAvailable` via `useAtomValue`
