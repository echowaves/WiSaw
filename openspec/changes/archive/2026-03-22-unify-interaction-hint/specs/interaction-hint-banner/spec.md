...
- **WHEN** the `InteractionHintBanner` component mounts and no relevant SecureStore key is set
...
#### Scenario: Component checks all legacy keys for backward compatibility
- **WHEN** the component mounts and checks SecureStore for hint status
- **THEN** it SHALL check three keys using Promise.all: `interactionHintShown`, `photoActionsHintShown`, `waveContextMenuTooltipShown`
- **THEN** if ANY of the three keys has a value, the banner SHALL NOT be displayed
