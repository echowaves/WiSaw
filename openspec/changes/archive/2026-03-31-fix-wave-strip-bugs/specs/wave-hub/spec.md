...
#### Scenario: WaveCard photo long press opens wave context menu
- **WHEN** a user long-presses a photo thumbnail inside a `WaveCard`'s `WavePhotoStrip`
- **THEN** `WaveCard` SHALL invoke the same `onLongPress(wave)` handler that the info bar and kebab menu use
- **THEN** the wave context menu (ActionMenu) SHALL be shown
