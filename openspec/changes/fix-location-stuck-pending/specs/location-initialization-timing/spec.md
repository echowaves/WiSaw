...
Note on prior requirements: the 5 second `Promise.race` around `getLastKnownPositionAsync` and the 10 second race around Phase 3 setup that this capability previously mandated are superseded by the revised `Location Provider Timeouts` requirement above. Investigation of `expo-location@57.0.11` native sources (iOS `LocationModule.swift` `getLastKnownPositionAsync`/`watchPositionImplAsync`, Android `LocationModule.kt` and `LocationHelpers.kt` `requestContinuousUpdates`) shows both are fast: the former is a synchronous cached-position read, the latter resolves as soon as the streamer/request is registered (streaming runs in a detached task on iOS; on Android the promise settles via `onRequestSuccess`/`onRequestFailed`). Neither produces the observed indefinite `pending`; the actual failure mode — a registered watcher that never delivers a fix — is covered by the first-fix watchdog in the `location-provider` capability. These race-timeout requirements were spec'd in the 2026-06-19 `location-initialization-fix` change, marked done, and never implemented, so no code migration is needed.
