# Keyboard Avoidance Specification

## Purpose
Keyboard avoidance ensures that all text inputs throughout the app remain visible and usable when the on-screen keyboard is open. The app uses `react-native-keyboard-controller` as its sole keyboard handling library, replacing all legacy/unmaintained alternatives.
