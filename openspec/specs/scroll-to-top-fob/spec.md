...
- **THEN** the FOB SHALL animate into view in the upper-right corner of the masonry layout
- **THEN** the FOB SHALL slide in from the right edge using a spring animation
...
The FOB SHALL automatically dismiss itself by sliding right off the screen edge after 3 seconds of no scroll activity. Each new downward scroll event past the threshold SHALL reset the 3-second timer.

#### Scenario: User stops scrolling for 3 seconds
- **WHEN** the FOB is visible and no scroll events occur for 3 seconds
- **THEN** the FOB SHALL animate out by sliding to the right and fading out
...
- **THEN** the masonry SHALL call `scrollToOffset({ offset: 0, animated: true })`
- **THEN** the FOB SHALL immediately begin its hide animation (slide right)
...
- **THEN** the FOB SHALL animate in again from the right edge
...
The FOB SHALL be a 40×40px circle positioned absolutely in the upper-right corner of the masonry container, 12px from the top and 12px from the right edge. It SHALL use `theme.HEADER_BACKGROUND` for the background and display an `Ionicons chevron-up` icon in `theme.TEXT_SECONDARY` color. It SHALL have a subtle shadow consistent with the app's existing elevated components.
...
- **THEN** it SHALL be positioned at top: 12, right: 12 within its parent container
