...
The system SHALL allow users to create a new wave and immediately add the current photo to it via the quick-actions modal or expanded photo view. When grouping is enabled, photos captured at upload time are also assigned to waves automatically — either the current active wave (if location fits) or a newly created one (if drifted).
...
#### Scenario: Auto-assigned wave at upload time
...
#### Scenario: Auto-created wave at upload time due to drift
- **WHEN** grouping is enabled and a user captures a photo
...
The system SHALL include wave membership data in the `getPhotoDetails` GraphQL query response. Photos captured while grouping is enabled are always assigned a wave on first upload — there is no "ungrouped" state for uploaded photos when grouping is active.
