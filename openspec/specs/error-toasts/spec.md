...
### Scenario: User dismisses modal by button
**Given** the error modal is open
**When** the user taps the [Dismiss] button or [✕] header button
**Then** the modal closes

### Scenario: User dismisses modal by backdrop
**Given** the error modal is open
**When** the user taps the backdrop overlay
**Then** the modal closes

### Scenario: New error replaces current modal content
**Given** the error modal is open showing error A
**When** `showErrorToast()` is called for error B
**Then** the modal content updates to show error B's details (modal stays open)

### Scenario: Error toast without stack trace
**Given** an error occurs without a stack trace (e.g., a simple string error)
**When** `showErrorToast()` is called with only `title` and `message`
**Then** the toast shows normally and the modal shows the error details without a stack trace section
